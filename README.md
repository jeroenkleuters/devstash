...
The seed creates `demo@CodeSquirrel.io` (password `12345678`) with the eight system item types and five collections holding 18 items. The demo account is seeded `isPro: true`, since five collections would otherwise sit over the free tier's cap of three.
...
`db:prune` is destructive: it deletes every user except `demo@CodeSquirrel.io` and
