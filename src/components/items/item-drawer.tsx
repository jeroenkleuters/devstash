...
function ItemContent({ detail }: { detail: ItemDetail }) {
  ...
  if (isCodeType(detail.type.slug)) {
    // The same frame, with Explain in its bar and the answer as a second tab.
    // Keyed on the item so clicking a second card — which swaps `detail`
    // without remounting the drawer — cannot leave the previous item's
    // explanation on screen; closing clears it too, Radix unmounting
    // `SheetContent`.
    return (
      <AiExplainableCode
        key={detail.id}
